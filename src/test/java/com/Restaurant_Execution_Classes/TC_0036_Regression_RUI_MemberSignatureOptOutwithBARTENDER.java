...
public class TC_0036_Regression_RUI_MemberSignatureOptOutwithBARTENDER {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir")
				+ "\\Resources\\Reports\\TC_0036_Regression_RUI_MemberSignatureOptOutwithBARTENDER.html";
		CurrentPageTestCaseName = "Member Signature OptOut with BARTENDER";
	...
	@Test
	public static void TC_0036_Regression_RUI_MemberSignatureOptOutwithBARTENDER()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_MemberSignatureCheckedOPtOut(row);
