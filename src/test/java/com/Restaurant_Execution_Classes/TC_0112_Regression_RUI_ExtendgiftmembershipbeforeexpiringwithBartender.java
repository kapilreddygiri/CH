...
public class TC_0112_Regression_RUI_ExtendgiftmembershipbeforeexpiringwithBartender  {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0112_Regression_RUI_ExtendgiftmembershipbeforeexpiringwithBartender.html";
		CurrentPageTestCaseName = "Extend Gift Membership to Monthly with BARTENDER";
	...
	@Test
	public static void TC_0112_Regression_RUI_ExtendgiftmembershipbeforeexpiringwithBartender ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_extendMembershipbeforeExpiry(row);
