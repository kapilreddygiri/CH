...
public class TC_0128_Regression_RUI_UpdateCreditCard  {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0128_Regression_RUI_UpdateCreditCard.html";
		CurrentPageTestCaseName = "TC_0128_Regression_RUI_UpdateCreditCard";
	...
	@Test
	public static void TC_0128_Regression_RUI_UpdateCreditCard ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_updateCreditCard(row);
