...
public class TC_0124_Regression_RUI_ConvertgiftmembershipafterexpiringwithBartender  {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0124_Regression_RUI_ConvertgiftmembershipafterexpiringwithBartender.html";
		CurrentPageTestCaseName = "Convert Gift Membership to Monthly with BARTNEDER";
	...
	@Test
	public static void TC_0124_Regression_RUI_ConvertgiftmembershipafterexpiringwithBartender ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_convertMembershipafterExpiry();
