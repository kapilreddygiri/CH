...
public class TC_0096_Regression_RUI_Changefrom2to1bottlemembershipNextBillingCyclewithBARTENDER  {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0096_Regression_RUI_Changefrom2to1bottlemembershipNextBillingCyclewithBARTENDER.html";
		CurrentPageTestCaseName = "Change from 2 to 1 bottle membership Next Billing Cycle with BARTENDER";
	...
	@Test
	public static void TC_0096_Regression_RUI_Changefrom2to1bottlemembershipNextBillingCyclewithBARTENDER ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WineClubInformation_Changebottle2to1nextBillingCycle(row);
