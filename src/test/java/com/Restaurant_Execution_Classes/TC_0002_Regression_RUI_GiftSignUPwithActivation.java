...
public class TC_0002_Regression_RUI_GiftSignUPwithActivation {
	...
	public static void Beforetest() throws IOException, BiffException {
		...
		HTMLReportPAth = System.getProperty("user.dir")
				+ "\\Resources\\Reports\\RestaurantMonthly_SignUPNoPayment.html";
		CurrentPageTestCaseName = "Restaurant Monthly Member SignUP No Payment";
	...
	@Test
	public static void RestaurantMonthly_SignUPNoPayment()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					try {
						System.out.println("In Main Catch Try");
 					} catch (Exception e1) {
