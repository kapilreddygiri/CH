...
public class TC_0075_Regression_RUI_ChangeClubtypeNextBillingcycleDecreasedcostwithTRA {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0075_Regression_RUI_ChangeClubtypeNextBillingcycleDecreasedcostwithTRA.html";
		CurrentPageTestCaseName = "Change Club type Next Billing cycle Decreased cost with TRA";
	...
	@Test
	public static void TC_0075_Regression_RUI_ChangeClubtypeNextBillingcycleDecreasedcostwithTRA()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WineClubInformation_DecreasedClubTypeMemberCostNextBillingCycle(row);
