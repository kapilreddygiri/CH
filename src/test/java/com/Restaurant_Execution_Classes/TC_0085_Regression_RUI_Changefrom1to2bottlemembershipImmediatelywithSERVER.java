...
public class TC_0085_Regression_RUI_Changefrom1to2bottlemembershipImmediatelywithSERVER  {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0085_Regression_RUI_Changefrom1to2bottlemembershipImmediatelywithSERVER.html";
		CurrentPageTestCaseName = "Change from 1 to 2 bottle membership Immediately with SERVER";
	...
	@Test
	public static void TC_0085_Regression_RUI_Changefrom1to2bottlemembershipImmediatelywithSERVER ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WineClubInformation_Changebottle1to2immediately(row);
