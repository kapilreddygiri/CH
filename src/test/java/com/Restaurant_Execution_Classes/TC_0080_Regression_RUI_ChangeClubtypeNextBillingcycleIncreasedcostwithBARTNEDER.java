...
public class TC_0080_Regression_RUI_ChangeClubtypeNextBillingcycleIncreasedcostwithBARTNEDER {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir")
				+ "\\Resources\\Reports\\TC_0080_Regression_RUI_ChangeClubtypeNextBillingcycleIncreasedcostwithBARTNEDER.html";
		CurrentPageTestCaseName = "Change Club type Next Billing cycle Increased cost with BARTENDER";
	...
	@Test
	public static void TC_0080_Regression_RUI_ChangeClubtypeNextBillingcycleIncreasedcostwithBARTNEDER()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WineClubInformation_IncreasedClubTypeMemberCostNextBillingCycle(row);
