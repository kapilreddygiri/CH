...
public class TC_0019_Regression_RUI_CancelMembershipwithTRA {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0019_Regression_RUI_CancelMembershipwithTRA.html";
		CurrentPageTestCaseName = "Cancel Membership with TRA";
	...
	@Test
	public static void TC_0019_Regression_RUI_CancelMembershipwithTRA()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_CancelMembership( );
