...
public class TC_0131_Regression_RUI_PickUpCurrentWOMUpchargewithBARTENDER {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0056_Regression_RUI_PickUpPriorWOMUpchargewithBARTENDER.html";
		CurrentPageTestCaseName = "PickUp Prior WOM No Upcharge with BARTENDER";
	...
	@Test
	public static void TC_0056_Regression_RUI_PickUpPriorWOMUpchargewithBARTENDER()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WinePickUp_SelectWCPriorWOMPickUPwithCharge();
