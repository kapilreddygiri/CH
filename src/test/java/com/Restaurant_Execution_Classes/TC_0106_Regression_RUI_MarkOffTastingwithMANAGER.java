...
public class TC_0106_Regression_RUI_MarkOffTastingwithMANAGER  {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0106_Regression_RUI_MarkOffTastingwithMANAGER.html";
		CurrentPageTestCaseName = " MarkOff Tasting with Manager";
	...
	@Test
	public static void TC_0106_Regression_RUI_MarkOffTastingwithMANAGER ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WinePickUp_markOffTasting();
