...
public class TC_0100_Regression_RUI_PutVacationHoldwithBARTENDER  {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0100_Regression_RUI_PutVacationHoldwithBARTENDER.html";
		CurrentPageTestCaseName = "Put Vacation Hold with BARTENDER";
	...
	@Test
	public static void TC_0099_Regression_RUI_PutVacationOnHoldwithTRA ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_VacationHold( );
 					break;
				} catch (Exception e) {
