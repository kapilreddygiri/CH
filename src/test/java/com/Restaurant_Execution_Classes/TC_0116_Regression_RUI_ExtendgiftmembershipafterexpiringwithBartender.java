...
public class TC_0116_Regression_RUI_ExtendgiftmembershipafterexpiringwithBartender  {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0116_Regression_RUI_ExtendgiftmembershipafterexpiringwithBartender.html";
	...
	@Test
	public static void TC_0116_Regression_RUI_ExtendgiftmembershipafterexpiringwithBartender ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_extendMembershipafterExpiry(row);
