...
public class TC_0108_Regression_RUI_Swap2WOMsfor1LuxwithBARTENDER {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0108_Regression_RUI_Swap2WOMsfor1LuxwithBARTENDER.html";
		CurrentPageTestCaseName = " Swap 2WOMs for 1 Lux with BARTENDER";
	...
	@Test
	public static void TC_0108_Regression_RUI_Swap2WOMsfor1LuxwithBARTENDER ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WinePickUp_swap2Womsfor1Lux();
