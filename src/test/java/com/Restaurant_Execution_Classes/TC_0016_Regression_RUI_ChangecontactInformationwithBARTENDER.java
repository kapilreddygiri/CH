...
public class TC_0016_Regression_RUI_ChangecontactInformationwithBARTENDER {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0016_Regression_RUI_ChangecontactInformationwithBARTENDER.html";
		CurrentPageTestCaseName = "Change contact Information with BARTENDER";
	...
	@Test
	public static void TC_0016_Regression_RUI_ChangecontactInformationwithBARTENDER()
		...
		for (int row = 1;row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_ChangeContactInformation(row);
