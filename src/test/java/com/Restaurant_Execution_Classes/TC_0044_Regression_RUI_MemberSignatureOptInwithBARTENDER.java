...
public class TC_0044_Regression_RUI_MemberSignatureOptInwithBARTENDER {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0044_Regression_RUI_MemberSignatureOptInwithBARTENDER.html";
		CurrentPageTestCaseName = "Member Signature OptIn with BARTENDER";
	...
	@Test
	public static void TC_0044_Regression_RUI_MemberSignatureOptInwithBARTENDER()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_MemberSignatureCheckedOPtIn( );
 					break;
				} catch (Exception e) {
