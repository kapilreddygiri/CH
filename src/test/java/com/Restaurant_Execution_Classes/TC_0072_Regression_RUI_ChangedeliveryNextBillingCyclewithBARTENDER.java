...
public class TC_0072_Regression_RUI_ChangedeliveryNextBillingCyclewithBARTENDER {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0072_Regression_RUI_ChangedeliveryNextBillingCyclewithBARTENDER.html";
		CurrentPageTestCaseName = "Change delivery Next Billing Cycle with BARTENDER";
	...
	@Test
	public static void TC_0072_Regression_RUI_ChangedeliveryNextBillingCyclewithBARTENDER()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WineClubInformation_ChangeDeliveryNextBillingCycle(row);
