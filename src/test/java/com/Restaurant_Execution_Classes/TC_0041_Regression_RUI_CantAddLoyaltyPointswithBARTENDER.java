...
public class TC_0041_Regression_RUI_CantAddLoyaltyPointswithBARTENDER {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir")
				+ "\\Resources\\Reports\\TC_0041_Regression_RUI_CantAddLoyaltyPointswithBARTENDER.html";
		CurrentPageTestCaseName = "Cant Add Loyalty Points with BARTENDER";
	...
	@Test
	public static void TC_0041_Regression_RUI_CantAddLoyaltyPointswithBARTENDER()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_Rewards_CannotAddLoyaltyPoints(row);
