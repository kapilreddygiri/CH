...
public class TC_0120_Regression_RUI_ConvertgiftmembershipbeforeexpiringwithBartender  {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0120_Regression_RUI_ConvertgiftmembershipbeforeexpiringwithBartender.html";
		CurrentPageTestCaseName = "Convert Gift Membership to Monthly with BARTENDER";
	...
	@Test
	public static void TC_0120_Regression_RUI_ConvertgiftmembershipbeforeexpiringwithBartender ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_convertMembershipbeforeExpiry();
