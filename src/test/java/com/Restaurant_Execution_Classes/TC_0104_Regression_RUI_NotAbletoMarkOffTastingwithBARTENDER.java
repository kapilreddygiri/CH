...
public class TC_0104_Regression_RUI_NotAbletoMarkOffTastingwithBARTENDER  {
	...
	public static void BeforeTest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0104_Regression_RUI_NotAbletoMarkOffTastingwithBARTENDER.html";
		CurrentPageTestCaseName = " Cannot MarkOff Tasting with BARTENDER";
	...
	@Test
	public static void TC_0104_Regression_RUI_NotAbletoMarkOffTastingwithBARTENDER ()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WinePickUp_noMarkOffTasting();
