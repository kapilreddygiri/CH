...
public class TC_0039_Regression_RUI_AddLoyaltyPointswithManager {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir")
				+ "\\Resources\\Reports\\TC_0039_Regression_RUI_AddLoyaltyPointswithManager.html";
		CurrentPageTestCaseName = "Add Loyalty Points with Manager";
	...
	@Test
	public static void TC_0039_Regression_RUI_AddLoyaltyPointswithManager()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_Rewards_AddLoyaltyPoints(row);
