...
public class TC_0024_Regression_RUI_AddNoteswithBARTENDER {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir")
				+ "\\Resources\\Reports\\TC_0024_Regression_RUI_AddNoteswithBARTENDER.html";
		CurrentPageTestCaseName = "Add Notes with BARTENDER";
	...
	@Test
	public static void TC_0024_Regression_RUI_AddNoteswithBARTENDER()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_Services_AddNotes(row);
