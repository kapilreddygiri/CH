...
public class TC00006_JulyPromo_TRAcanonlyviewPromobottle {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC00006_JulyPromo_TRAcanonlyviewPromobottle.html";
		CurrentPageTestCaseName = "TC00006_JulyPromo_TRAcanonlyviewPromobottle";
	...
	@Test
	public static void TC00006_JulyPromo_TRAcanonlyviewPromobottle()
		...
		for (int row = 2; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WinePickUp_JulyPromoNotEnabled();
