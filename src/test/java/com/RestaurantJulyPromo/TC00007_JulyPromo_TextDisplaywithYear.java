...
public class TC00007_JulyPromo_TextDisplaywithYear {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC00007_JulyPromo_TextDisplaywithYear.html";
		CurrentPageTestCaseName = "TC00007_JulyPromo_TextDisplaywithYear";
	...
	@Test
	public static void TC00007_JulyPromo_TextDisplaywithYear()
		...
		for (int row = 2; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WinePickUp_julyPromoDisplayedwithYear();
