...
public class TC00003_RUI_JulyPromo_BasicPickLimitation {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir")
				+ "\\Resources\\Reports\\TC00003_RUI_JulyPromo_BasicPickLimitation.html";
		CurrentPageTestCaseName = "TC00003_RUI_JulyPromo_BasicPickLimitation";
		ExcelSheetPath = System.getProperty("user.dir") + "\\Resources\\TestData\\JulyPromo_Restaurant_TestData.xls";
	...
	@Test
	public static void TC00003_RUI_JulyPromo_BasicPickLimitation()
		...
		for (int row = 1; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WinePickUp_SelectJulypromo(row);
