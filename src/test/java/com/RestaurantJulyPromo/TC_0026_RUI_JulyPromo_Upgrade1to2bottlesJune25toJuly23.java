...
public class TC_0026_RUI_JulyPromo_Upgrade1to2bottlesJune25toJuly23 {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0026_RUI_JulyPromo_Upgrade1to2bottlesJune25toJuly23.html";
	...
	@Test
	public static void TC_0026_RUI_JulyPromo_Upgrade1to2bottlesJune25toJuly23()
		...
		for (int row = 2; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_AccountInfo_bottlesUpgradefrom1to2(row);
