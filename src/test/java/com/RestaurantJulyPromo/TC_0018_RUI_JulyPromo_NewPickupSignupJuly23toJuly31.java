...
public class TC_0018_RUI_JulyPromo_NewPickupSignupJuly23toJuly31 {
	...
	public static void Beforetest() throws IOException, BiffException {
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\TC_0018_RUI_JulyPromo_NewPickupSignupJuly23toJuly31.html";
		CurrentPageTestCaseName = " July Promo New Pickup Signup from June23 to July31";
	...
	@Test
	public static void TC_0018_RUI_JulyPromo_NewPickupSignupJuly23toJuly31()
		...
		for (int row = 2; row < RestaurantMonthlyMemberData.GetRows(); row++) {
			if (RestaurantMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					Restaurant_Common_Pages.Page_WinePickUp_JulyPromoEligibilityforNewPickUpTypeSignUpJuly23toJuly31(row);
