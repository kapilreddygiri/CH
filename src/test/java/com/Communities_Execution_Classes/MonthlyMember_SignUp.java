...
public class MonthlyMember_SignUp {
	...
	public static void Beforetest() throws IOException, BiffException {
		...
		HTMLReportPAth = System.getProperty("user.dir") + "\\Resources\\Reports\\CommunitiesMonthlyMembers.html";
		CurrentPageTestCaseName = "Communities Monthly Member SignUP";
	...
	@Test
	public static void CommunitiesMonthlyMemberSignUP()
		...
		for (int row = 1; row < CommunitiesMonthlyMemberData.GetRows(); row++) {
			if (CommunitiesMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					CommunitiesPages.CMUMonthly_EnterWineClubInformation(row);
					CommunitiesPages.CMUPage_PaymentPage_EnterandValidate_PaymentPage(row);
					CommunitiesPages.CMUPage_PaymentApproved_CreatePasswordandCopyCardNumber(row);
					CommunitiesPages.CMUPage_Dashboard_ValidateDashboard(row);
					CommunitiesPages.CMUPage_Profile_ValidateProfile(row);
