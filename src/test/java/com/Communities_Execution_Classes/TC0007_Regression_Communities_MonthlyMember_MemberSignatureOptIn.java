...
public class TC0007_Regression_Communities_MonthlyMember_MemberSignatureOptIn {
	...
	public static void Beforetest() throws IOException, BiffException {
		...
		HTMLReportPAth = System.getProperty("user.dir")
				+ "\\Resources\\Reports\\TC0007_Regression_Communities_MonthlyMember_MemberSignatureOptIn.html";
		CurrentPageTestCaseName = "TC0007_Regression_Communities_MonthlyMember_MemberSignatureOptIn";
	...
	@Test
	public static void TC0007_Regression_Communities_MonthlyMember_MemberSignatureOptIn()
		...
		for (int row = 1; row < CommunitiesMonthlyMemberData.GetRows(); row++) {
			if (CommunitiesMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					CommunitiesPages.CMUPage_Profile_memberSignatureOptIn();
