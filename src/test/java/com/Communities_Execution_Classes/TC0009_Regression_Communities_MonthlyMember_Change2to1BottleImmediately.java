...
public class TC0009_Regression_Communities_MonthlyMember_Change2to1BottleImmediately {
	...
	public static void Beforetest() throws IOException, BiffException {
		...
		HTMLReportPAth = System.getProperty("user.dir")
				+ "\\Resources\\Reports\\TC0009_Regression_Communities_MonthlyMember_Change2to1BottleImmediately.html";
		CurrentPageTestCaseName = "TC0009_Regression_Communities_MonthlyMember_Change2to1BottleImmediately";
	...
	@Test
	public static void TC0009_Regression_Communities_MonthlyMember_Change2to1BottleImmediately()
		...
		for (int row = 1; row < CommunitiesMonthlyMemberData.GetRows(); row++) {
			if (CommunitiesMonthlyMemberData.Getdata("WC Member Record Type", row).trim().length() > 2) {
				try {
					...
					CommunitiesPages.CMUPage_Profile_change2to1Bottleimmediately();
